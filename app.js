...
(function () {
  ...
  async function initNativeNotifications() {
    ...
    try {
      ...
      await plugin.createChannel({
        id: 'campusflow-tasks-v2',
        name: '任务步骤提醒',
        description: '提醒当前任务应该完成的下一步',
        importance: 5,
        visibility: 1,
        vibration: true,
        sound: 'campusflow_reminder.wav'
      });
  ...
  async function checkReminders(options) {
    ...
    const notification = notificationStatus();
    // Keep the user's intent even when the OS/browser permission is later
    // revoked. The effective channel stays disabled until permission is
    // restored, but silently rewriting this setting caused Android startup
    // races to permanently turn reminders off.
  ...
  function calendarDayItems(key) {
    const events = state.events.filter((item) => !item.autoGenerated && item.date === key).map((item) => ({ ...item, kind: '事件' }));
    const tasks = state.settings.autoEvent === true
      ? state.tasks.filter((item) => item.status !== 'done' && item.due === key).map((item) => ({ ...item, kind: '任务', date: item.due }))
      : [];
    return [...events, ...tasks].sort((a, b) => String(a.time || '').localeCompare(String(b.time || '')));
  }

  function openCalendarDay(key) {
    const items = calendarDayItems(key);
    if (!items.length) {
      openModal('新建日历事件', formatDate(key), eventForm({ date: key }));
      return;
    }
    if (items.length === 1) {
      const [item] = items;
      if (item.kind === '任务') openModal('编辑任务', '修改后会立即同步到总览与日历。', taskForm(state.tasks.find((task) => task.id === item.id)));
      else openModal('编辑日历事件', '修改后会同步到日历视图。', eventForm(state.events.find((event) => event.id === item.id)));
      return;
    }
    const rows = items.map((item) => `<button class="calendar-detail-item" data-action="${item.kind === '任务' ? 'edit-task' : 'edit-event'}" data-id="${escapeHtml(item.id)}"><span class="calendar-detail-time">${escapeHtml(item.time || '全天')}</span><span><strong>${escapeHtml(item.title)}</strong><small>${item.kind} · ${escapeHtml(item.place || item.category || '未分类')}</small></span><b>›</b></button>`).join('');
    openModal(formatDate(key), `当天共有 ${items.length} 项安排，点一项查看详情。`, `<div class="calendar-detail-list">${rows}</div><div class="modal-foot"><button type="button" class="btn btn-ghost" data-action="close-modal">关闭</button><button type="button" class="btn btn-primary" data-action="calendar-add-event" data-date="${key}">＋ 添加事件</button></div>`);
  }
  ...
  async function bootstrapApp() {
    render();
    syncSidebarA11y();
    // Android permission must be known before the first reminder check. If
    // the check runs while permission is still "unknown", it can otherwise
    // mistake an enabled setting for a revoked permission and turn it off.
    if (isNativeApp()) await initNativeNotifications();
    initReminderChecks();
  }
  bootstrapApp();
